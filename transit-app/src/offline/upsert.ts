 ...
 * @param chunkSize - Number of rows to insert at once to keep each BroadcastChannel RPC (pglite worker) payload under limit
 *
 * @example
...
export async function upsertMany<T extends PgTable>(
    ...
    return await Promise.all(
        Array.from({ length: Math.ceil(values.length / chunkSize) }, (_, i) =>
            db
                .insert(table)
                .values(values.slice(i * chunkSize, (i + 1) * chunkSize))
