...
const router = new Hono()
    ...
    .get(
        ...
        zValidator(
            "query",
            z.object({ directionId: z.enum(["0", "1"]).transform(Number).optional() })
        ),
        async (c) => {
                ...
                try {
                    ...
                    try {
                        await stream.writeSSE({
                            ...
                            data: JSON.stringify({
                                message:
                                    error?.message || "An internal error occurred in the stream.",
                            }),
                        });
                    } catch (writeError) {
                        console.error(
                            `Failed to write error SSE for route ${routeId}:`,
                            writeError
                        );
                    }
                } finally {
