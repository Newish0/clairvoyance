...
class GTFSReader:
    ...
    def __init__(
        self, default_timezone: str = "UTC", logger: Optional[logging.Logger] = None
    ):
        """
    ...
    def parse(self, static_data_source: DataSource) -> ParsedGTFSData:
        self.logger.info(f"Starting to read GTFS data from {static_data_source}")
        zip_file_data = get_resource(static_data_source, logger=self.logger)
    ...
    def _read_gtfs_zip(self, zip_file_buffer: io.BytesIO) -> Tuple[Dict[str, Any], ...]:
        ...
        Args:
            zip_file_buffer: A BytesIO object containing the zip file data.
        ...
        try:
            ...
            self.logger.error(f"Invalid zip file")
            raise ValueError(f"Invalid zip file")
    ...
    def _read_csv_from_zip(
        ...
        try:
            ...
            text_content = io.TextIOWrapper(
                io.BytesIO(file_content), encoding="utf-8-sig"
            )
                ...
                try:
                    for row in reader:
                        ...
                        if route_id:
                            ...
                            self.logger.warning(
                                f"Found route without route_id in {filename}"
                            )
                ...
                try:
                    for row in reader:
                        ...
                        if trip_id and service_id:
                            ...
                            self.logger.debug(
                                f"Skipping trip with missing trip_id or service_id: {row}"
                            )
                    self.logger.info(f"Parsed {trip_count} trips from {filename}")
                    if missing_data_count > 0:
                        self.logger.warning(
                            f"Skipped {missing_data_count} trips with missing required data"
                        )
                except Exception as e:
                    self.logger.error(f"Error parsing {filename}: {e}", exc_info=True)
            else:
    ...
    def _parse_calendar(
        ...
        if filename in filenames:
            ...
            if reader:
                ...
                try:
                    for row in reader:
                        ...
                        if not service_id:
                            self.logger.warning(
                                f"Found calendar entry without service_id in {filename}"
                            )
                            continue
                        ...
                        try:
                            ...
                            self.logger.debug(
                                f"Service {service_id}: Found {date_count} active dates from {start_date} to {end_date} via {filename}"
                            )
                            service_count += 1

                        except (ValueError, TypeError) as e:
                            self.logger.warning(
                                f"Invalid date format or missing data in {filename} for service {service_id}: {e} - Row: {row}"
                            )
                            error_count += 1

                    self.logger.info(
                        f"Parsed {service_count} services from {filename} with {error_count} errors"
                    )
                except Exception as e:
                    self.logger.error(f"Error parsing {filename}: {e}", exc_info=True)
            else:
            ...
            self.logger.info(
                f"{filename} not found. Relying on calendar_dates.txt if present."
            )
                ...
                try:
                    for row in reader:
                        ...
                        if not (service_id and date_str and exception_type):
                            self.logger.warning(
                                f"Skipping {filename} entry with missing required fields: {row}"
                            )
                            error_count += 1
                            continue
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Invalid date format '{date_str}' in {filename} for service {service_id}. Skipping entry."
                            )
                            error_count += 1
                            continue
                        ...
                        try:
                            ...
                            if exception_type == "1":
                                ...
                                self.logger.warning(
                                    f"Unknown exception_type: {exception_type} for service {service_id} on date {date_str} in {filename}"
                                )
                                error_count += 1

                            service_dates[service_id] = sorted(
                                list(current_dates_for_service)
                            )  # Store back as sorted list

                        except Exception as e:
                            self.logger.warning(
                                f"Error processing {filename} entry: {row} - {e}"
                            )
                            error_count += 1

                    self.logger.info(
                        f"Processed {exception_count} exceptions from {filename} ({added_count} added, {removed_count} removed, {error_count} errors)"
                    )
                except Exception as e:
                    self.logger.error(f"Error parsing {filename}: {e}", exc_info=True)
            else:
                ...
                try:
                    ...
                    for row in reader:
                        ...
                        if trip_id:
                            ...
                            self.logger.warning(
                                f"Found stop time without trip_id in {filename}"
                            )
                            error_count += 1

                    self.logger.debug(
                        f"Collected {stop_time_count} raw stop time entries"
                    )
                    ...
                    try:
                        all_stop_times_raw.sort(
                            key=lambda x: (
                                ...
                                (
                                    int(x.get("stop_sequence", "0"))
                                    if x.get("stop_sequence", "").isdigit()
                                    else float("inf")
                                ),
                            )
                        )
                        self.logger.debug(
                            f"Successfully sorted {stop_time_count} stop times by trip_id and stop_sequence"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"Error sorting stop times: {e}. Grouping may be affected.",
                            exc_info=True,
                        )
                    ...
                    self.logger.info(
                        f"Grouped {stop_time_count} stop times for {len(trips_with_stops)} trips from {filename}"
                    )
                    if error_count > 0:
                        self.logger.warning(
                            f"Encountered {error_count} errors (e.g., missing trip_id) while parsing {filename}"
                        )
                ...
                try:
                    for row in reader:
                        ...
                        if stop_id:
                            ...
                            self.logger.warning(
                                f"Found stop without stop_id in {filename}: {row}"
                            )
                            error_count += 1
                    self.logger.info(f"Parsed {stop_count} stops from {filename}")
                    if error_count > 0:
                        self.logger.warning(
                            f"Skipped {error_count} stops due to missing stop_id in {filename}"
                        )
                except Exception as e:
                    self.logger.error(f"Error parsing {filename}: {e}", exc_info=True)
            else:
    ...
    def _parse_shapes(
        ...
        if filename in filenames:
                    ...
                    for row in reader:
                        ...
                        if shape_id:
                            ...
                            try:
                                ...
                                self.logger.warning(
                                    f"Skipping shape point due to invalid/missing required fields (lat, lon, seq) in {filename}: {valid_err} - Row: {row}"
                                )
                                error_count += 1
                        else:
                            self.logger.warning(
                                f"Found shape point without shape_id in {filename}: {row}"
                            )
                            error_count += 1
                    self.logger.info(
                        f"Parsed {point_count} shape points for {len(shape_ids_found)} distinct shapes from {filename}"
                    )
                    if error_count > 0:
                        self.logger.warning(
                            f"Skipped {error_count} shape points due to errors in {filename}"
                        )
            ...
            self.logger.info(
                f"{filename} not found in GTFS zip file."
            )  # Info level might be okay here
        ...
        for shape_id in shapes_by_id:
            try:
                shapes_by_id[shape_id].sort(
                    key=lambda x: int(
                        x.get("shape_pt_sequence", "0")
                    )  # Graceful handling of missing sequence
                )
            except ValueError:
                self.logger.error(
                    f"Could not sort shape points for shape_id {shape_id} due to non-integer sequence numbers. Shape geometry might be incorrect."
                )
