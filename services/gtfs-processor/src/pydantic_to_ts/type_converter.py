...
def _python_type_to_typescript_type(py_type: any, module_members: dict) -> str:
    ...
    if origin:
        ...
        if origin is dict:
            if args and len(args) == 2:
                ...
                ts_key_type_str = _python_type_to_typescript_type(key_type_py, module_members)
                ts_value_type_str = _python_type_to_typescript_type(value_type_py, module_members)
                ...
                if ts_key_type_str not in ["string", "number"] and not ('"' in ts_key_type_str or "'" in ts_key_type_str or '`' in ts_key_type_str): # Crude check for string/number literals
                    # This check means if ts_key_type_str is "MyEnum", it will be changed to "string".
                    # If it was Literal['A' | 'B'], it would pass.
                    actual_key_type_repr = getattr(key_type_py, "__name__", str(key_type_py))
                    print(
        ...
        if is_python_union:
            ...
            ts_types_for_union = [
                _python_type_to_typescript_type(arg, module_members) for arg in non_none_args
            ]
            
            if type(None) in args: # If original Union included NoneType
                if "null" not in ts_types_for_union: # Avoid duplicates like "string | null | null"
                    ts_types_for_union.append("null")
            
            # Remove duplicates and sort for consistent output
            unique_sorted_ts_types = sorted(list(set(ts_types_for_union)))
            
            if not unique_sorted_ts_types: # Should not happen if non_none_args had items
                 return "any" # Fallback
    ...
    if inspect.isclass(py_type):
        ...
        if py_type.__name__ in module_members and \
           py_type is module_members.get(py_type.__name__): # Ensure it's the exact object
...
def _convert_pydantic_model_to_typescript(
    ...
    for field_name, field_info in model_cls.model_fields.items():
        ...
        is_opt = _is_optional(field_info) # Pydantic V2: checks field_info.is_required()
...
def generate_typescript_defs(module_path: str, output_ts_file: str) -> None:
    ...
    final_ts_code = (
        ...
        + "\n\n"
        + "\n\n".join(filter(None, ts_outputs)) # Filter out any empty strings from conversions
    )
